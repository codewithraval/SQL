/* One of the most powerful features added in MySQL version 8.0 is common table expressions, which allow for the construction of 
...
select * from (SELECT department, 
           ...
           AVG(salary) AS average_salary
    FROM employees
    GROUP BY department) as a where average_salary > 60000;
...
SELECT salesperson_id, total_sales
FROM (SELECT salesperson_id, SUM(sales_amount) AS total_sales
    FROM sales
    GROUP BY salesperson_id) as a where total_sales > 5000;
