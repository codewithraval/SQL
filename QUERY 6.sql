...
UPDATE BANK SET EDUCATION = 'GRADUTION' WHERE EDUCATION = 'UNKNOWN';

SELECT MIN(BALANCE) FROM BANK;

SELECT LOAN,SUM(BALANCE) FROM BANK GROUP BY LOAN;

SELECT JOB,MIN(BALANCE) FROM BANK GROUP BY JOB; 

SELECT * FROM BANK WHERE AGE = 33 AND MARITAL = 'MARRIED' ;

SELECT * FROM BANK WHERE EDUCATION  = 'GRADUTION' OR MARITAL = 'SINGLE' OR BALANCE < 500; 

SELECT * FROM BANK WHERE EDUCATION  = 'GRADUTION' AND MARITAL = 'SINGLE' AND BALANCE < 500;
...
(1,'ALICE JOHNSON', 1, '2015-03-01', 60000),
(2,'BOB SMITH', 2, '2018-07-15', 50000),
(3,'CATHERINE BROWN', 1, '2020-10-20', 45000),
(4,'DAVID WILSON', 3, '2012-09-05', 75000),
(5,'EMILY DAVIS', 2, '2019-11-01', 55000);
...
CREATE TABLE DEPARTMENTS (
 ...
 (6, 1,'2023-01-02','ABSENT'),
 (7, 2,'2023-01-01','PRESENT');

/* FIND DEPATMENT WITH TWO OR MORE THEN TWO EMPLOYEES WHO */
SELECT D.DEPARTMENTNAME,A.DATE,COUNT(A.EMPLOYEEID) AS ABSENTCOUNT
FROM ATTENDANCE AS A 
INNER JOIN EMPLOYEE AS E 
ON A.EMPLOYEEID = E.EMPLOYEEID
INNER JOIN DEPARTMENTS AS D
ON E.DEPATMENTID = D.DEPARTMENTID
WHERE 
  A.STATUS = 'ABSENT'
  GROUP BY D.DEPARTMENTNAME,A.DATE
...
(1,'ALICE','SMITH','ALICR.SMITH@EXAMLE.COM'),
...
/* LIST OF PRODUCTS PARCHASED BY A SPECIFIC CUSTOMER*/
...
/* FIND CUSTOMER WHO HAVE ORDERED A LAPTOL*/
...
/* IT IS A FUNTION ARE USE TEMPRERY( ONLY SHOW OUTPUT) NOT CHANGE THE DATA */

/* STRING FUNTION */

USE SAKILA;

SELECT CONCAT(FIRST_NAME,'  ',LAST_NAME) AS FULL_NAME FROM CUSTOMER; # CAMBAIN TWO COLUMN

SELECT LOWER(FIRST_NAME) FROM CUSTOMER;

SELECT UPPER(FIRST_NAME) FROM CUSTOMER;

SELECT TRIM('BHARGAV RAVAL')AS NEW_NAME; # DELETE SPACE IS NOT USEFULL

SELECT LENGTH(FIRST_NAME) FROM CUSTOMER; # IT GIVE LENTH OF CHARCTER

SELECT SUBSTRING(FIRST_NAME,2,5) FROM CUSTOMER; # IT GIVE SELECTED LENTH

SELECT REPLACE('BHARGAV RAVAL','RAVAL','PRAJAPATI') AS NAME FROM CUSTOMER; # REPLACE NAME TEMPARERY

SELECT * FROM CUSTOMER;

/* DATE FUNTION */

SELECT DATE(CREATE_DATE) AS DATE FROM CUSTOMER;

SELECT TIME(CREATE_DATE) AS TIME FROM CUSTOMER;

SELECT YEAR(CREATE_DATE) AS YEAR FROM CUSTOMER;

SELECT MONTH(CREATE_DATE) AS MONTH FROM CUSTOMER;

SELECT DAY(CREATE_DATE) AS DAY FROM CUSTOMER;

SELECT CAST('2024-12-30' AS DATE ) AS NEW_DATE; # CHANGE DATATYPE TEMPARERY

SELECT NOW() AS CRT_DATE;

SELECT CURRENT_DATE() AS CRT_DATE;

SELECT PAYMENT_DATE,LAST_UPDATE, DATEDIFF(PAYMENT_DATE,LAST_UPDATE) AS DIFFERENCE FROM PAYMENT; # DIFFERENCE CHACK KARVA

SELECT IFNULL(AMOUNT,0) FROM PAYMENT; # REPLACE THE NULL VALUE

SELECT ROUND(AMOUNT,1) AS AMOUNT FROM PAYMENT; 
